...
def add_category_tags(database_id: str, 
    ...
    # Add new tags with optional color
    new_options = existing_options.copy()
    
    tags_to_add = []
    for tag in all_tags:
        if tag not in existing_names:
            # If color specified, use it for all tags, otherwise let Notion decide
            new_tag = {"name": tag}
            if tag_color:
                new_tag["color"] = tag_color
            
            new_options.append(new_tag)
    ...
    # Step 2: Fetch pages for all genes in a single query
    print(f"\n2. Fetching pages for {len(gene_to_categories)} specific genes in a single query...")
    
    query_url = f"https://api.notion.com/v1/databases/{database_id}/query"
    all_pages = []
    genes_to_process = list(gene_to_categories.keys())
    
    # First, determine which property contains gene names
    print("  Detecting gene name property...")
    rate_limiter.wait_if_needed()
    
    # Get a sample page to identify the correct property
    sample_query = {
        "page_size": 1
    }
    sample_response = requests.post(query_url, headers=headers, json=sample_query)
    
    gene_property = None
    gene_property_type = None
    
    if sample_response.status_code == 200:
        sample_data = sample_response.json()
        if sample_data.get("results"):
            sample_page = sample_data["results"][0]
            properties = sample_page.get("properties", {})
            
            # Look for the property containing gene names
            for prop_name in ["gene_name", "Gene Name", "Gene", "Name", "Title"]:
                if prop_name in properties:
                    prop = properties[prop_name]
                    if prop["type"] in ["title", "rich_text", "select"]:
                        gene_property = prop_name
                        gene_property_type = prop["type"]
                        print(f"  Found gene property: '{gene_property}' (type: {gene_property_type})")
                        break
    
    if not gene_property:
        print("  Warning: Could not detect gene property name. Trying 'gene_name' with both types...")
        gene_property = "gene_name"
        gene_property_type = None
    
    # Build OR filters for all genes, respecting Notion's 100 condition limit
    # Since we know the property type, we only need 1 condition per gene
    conditions_per_gene = 1  # We detected the property type above
    # Use smaller batches to avoid timeouts 
    max_genes_per_batch = min(50, 100 // conditions_per_gene)  # Max 50 genes per batch
    
    # Split genes into batches
    gene_batches = []
    for i in range(0, len(genes_to_process), max_genes_per_batch):
        gene_batches.append(genes_to_process[i:i + max_genes_per_batch])
    
    print(f"  Splitting into {len(gene_batches)} batch(es) due to API limits...")
    
    # Query each batch of genes
    for batch_idx, gene_batch in enumerate(gene_batches, 1):
        # Build OR conditions for this batch (only for detected property type)
        or_conditions = []
        for gene_name in gene_batch:
            if gene_property_type == "title":
                or_conditions.append({
                    "property": gene_property,
                    "title": {
                        "equals": gene_name
                    }
                })
            elif gene_property_type == "rich_text":
                or_conditions.append({
                    "property": gene_property,
                    "rich_text": {
                        "equals": gene_name
            ...
            elif gene_property_type == "select":
                or_conditions.append({
                    "property": gene_property,
                    "select": {
                        "equals": gene_name
                ...
                # Fallback: try rich_text which is most common
                print(f"  Warning: Unknown property type, defaulting to rich_text")
                or_conditions.append({
                    "property": gene_property,
                    "rich_text": {
                        "equals": gene_name
        ...
        # Track which genes we've found to stop early if possible
        batch_genes_found = set()
        batch_genes_needed = set(gene_batch)
        ...
        while has_more:
            ...
            print(f"  Batch {batch_idx}/{len(gene_batches)}, page {page_num}: ", end="")
            ...
            while retry_count < max_retries and not success:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        # Track genes found in this response
                        for page in batch_pages:
                            gene_name = None
                            properties = page.get("properties", {})
                            if gene_property in properties:
                                prop = properties[gene_property]
                                ...
                                if gene_name:
                                    batch_genes_found.add(gene_name)
                        
                        all_pages.extend(batch_pages)
                        
                        print(f"got {len(batch_pages)} pages, found {len(batch_genes_found)}/{len(batch_genes_needed)} genes")
                        
                        has_more = data.get("has_more", False)
                        start_cursor = data.get("next_cursor")
                        success = True
                    elif response.status_code in [502, 503, 504]:  # Gateway errors
                        retry_count += 1
                        if retry_count < max_retries:
                            wait_time = 2 ** retry_count  # Exponential backoff
                            print(f"\n  Gateway timeout, retrying in {wait_time}s... (attempt {retry_count}/{max_retries})")
                            time.sleep(wait_time)
                        else:
                            print(f"\n  Error after {max_retries} retries: {response.status_code}")
                            has_more = False
                    else:
                        print(f"\n  Error: {response.status_code} - {response.text}")
                        has_more = False
                        success = True  # Don't retry on non-gateway errors
                except requests.exceptions.Timeout:
                    retry_count += 1
                    if retry_count < max_retries:
                        wait_time = 2 ** retry_count
                        print(f"\n  Request timeout, retrying in {wait_time}s... (attempt {retry_count}/{max_retries})")
                        time.sleep(wait_time)
                    else:
                    ...
                    has_more = False
                    success = True  # Don't retry on other exceptions
    ...
    not_found = set(genes_to_process) - found_genes
    if not_found:
        print(f"  Genes not found in database: {', '.join(sorted(not_found))}")
...
def add_tags_to_gene_list():
    ...
    parser.add_argument('--token', type=str, required=False, default=os.environ.get('NOTION_TOKEN', os.environ.get('NOTION_API_KEY')), help="Notion integration token")
    parser.add_argument('--column', type=str, required=False, help="Multiselect column in Notion database to add tags to")
    parser.add_argument('--color', type=str, required=False, choices=['red', 'blue', 'green', 'yellow', 'purple', 'pink', 'orange', 'brown', 'gray'], help="Color for all tags (if not specified, Notion decides)")
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument('--yaml', type=Path, help="Yaml file with gene categories")
    group.add_argument('--sheet', type=str, action='append', help="Name of google sheet column")   
    ...
    if args.yaml:
        ...
        # Handle the format where genes are listed without hyphens
        gene_tags = {}
        for category, genes in data.items():
            if isinstance(genes, list):
                # Proper YAML list with hyphens
                gene_tags[category] = genes
            elif isinstance(genes, str):
                # Multi-line string without hyphens - split by whitespace (spaces AND newlines)
                gene_list = [g.strip() for g in genes.strip().split() if g.strip()]
                gene_tags[category] = gene_list
            else:
                gene_tags[category] = []
    elif args.sheet:
        ...
        gene_tags = {s: list(sheet.get(s)) for s in args.sheet}
    else:
        assert False, "Either --yaml or --sheet must be specified"

    add_category_tags(
        database_id=args.database_id,
        token=args.token,
        gene_categories=gene_tags,
        multiselect_column=args.column if args.column else "protein_gene_sets",
        batch_size=10,
        dry_run=False,
        tag_color=args.color
    )
